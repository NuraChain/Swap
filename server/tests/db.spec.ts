...
// to bring the exact bigint back. A silently truncated amount is a wrong price
...
const OTHER_POOL = '0x00000000000000000000000000000000000000bb' as Address;
...
/** uint256 max - the largest amount an event can carry. */
...
describe('schema migration', () =>
{
    ...
    it('adds columns to a database written before they existed', () =>
    {
        ...
        const pool = db.getV3Pool(OTHER_POOL);
        expect(pool?.fee).toBe(500);
        expect(pool?.balance0).toBe(0n);
        expect(pool?.sqrtPriceX96).toBe(0n);
    });

    // Databases written by the dual-protocol indexer carry a `pairs` table and a
    // `protocol` column this build never reads. The migration sheds both in
    // place instead of refusing to open the old file.
    it('drops the V2 relics from a dual-protocol database', () =>
    {
        const path = scratchPath();
        const old = new DatabaseSync(path);
        old.exec(`
        ...
        `);
        old.prepare('INSERT INTO events (block_number, log_index, tx_hash, ts, pair, protocol, kind, account, amount0_in) '
            + "VALUES (1, 0, '0xold', 100, ?, 'v2', 'swap', ?, '5')").run(POOL, ALICE);
        old.close();

        const db = new IndexerDb(path);
        open.push(db);

        // The row survives; its V2-only column does not.
        const [row] = db.recentEvents(10);
        expect(row.amount0In).toBe(5n);
        expect((row as unknown as Record<string, unknown>).protocol).toBeUndefined();
        // And inserts work again - the dropped column had a DEFAULT that would
        // have swallowed writes aimed at the narrower new schema.
    ...
    it('is idempotent - a second open of a migrated database changes nothing', () =>
    {
        ...
        first.upsertV3Pool({ address: OTHER_POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 3 });
        first.updateV3Price(OTHER_POOL, 12345n);
        first.close();

        const second = new IndexerDb(path);
        open.push(second);
        expect(second.getV3Pool(OTHER_POOL)?.sqrtPriceX96).toBe(12345n);
    });
});
...
describe('v3 pools', () =>
{
    it('stores a pool and reads it back', () =>
    {
        const db = freshDb();
        db.upsertV3Pool({ address: POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 7 });
        const row = db.getV3Pool(POOL);
        expect(row).not.toBeNull();
        expect(row?.token0).toBe(NURA);
        expect(row?.token1).toBe(USDT);
        expect(row?.fee).toBe(500);
        expect(row?.createdBlock).toBe(7);
        // A pool that has never been refreshed reads zero, not null or NaN.
        expect(row?.balance0).toBe(0n);
        expect(row?.balance1).toBe(0n);
        expect(row?.sqrtPriceX96).toBe(0n);
    });

    // PoolCreated is emitted once per pool, but the tail gets replayed on restart
    // and after a reorg rewind. A second insert must not reset state.
    it('ignores a duplicate creation instead of clobbering live balances', () =>
    {
        const db = freshDb();
        db.upsertV3Pool({ address: POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 7 });
        db.updateV3Balances(POOL, 100n, 200n);
        db.updateV3Price(POOL, 999n);
        db.upsertV3Pool({ address: POOL, token0: USDT, token1: NURA, fee: 3000, createdBlock: 9 });
        const row = db.getV3Pool(POOL);
        expect(row?.balance0).toBe(100n);
        expect(row?.balance1).toBe(200n);
        expect(row?.sqrtPriceX96).toBe(999n);
        expect(row?.token0).toBe(NURA);
        expect(row?.fee).toBe(500);
        expect(row?.createdBlock).toBe(7);
    });
    ...
    it('normalizes address case across creation, update and lookup', () =>
    {
        ...
        db.updateV3Balances(POOL.toUpperCase(), 5n, 6n);
        expect(db.getV3Pool(POOL)?.balance0).toBe(5n);
        expect(db.getV3Pool(POOL)?.token0).toBe(NURA);
    });

    it('holds full uint256 balances without truncating them', () =>
    {
        // SQLite's native integer is i64; uint256 does not fit. This is the whole
        // reason balances are TEXT columns.
        const db = freshDb();
        db.upsertV3Pool({ address: POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 1 });
        db.updateV3Balances(POOL, MAX_UINT256, MAX_UINT256 - 1n);
        expect(db.getV3Pool(POOL)?.balance0).toBe(MAX_UINT256);
        expect(db.getV3Pool(POOL)?.balance1).toBe(MAX_UINT256 - 1n);
    });

    it('treats a balance update for an unknown pool as a no-op', () =>
    {
        const db = freshDb();
        expect(() => db.updateV3Balances(POOL, 1n, 2n)).not.toThrow();
        expect(db.getV3Pool(POOL)).toBeNull();
    });

    it('lists pools in creation order', () =>
    {
        const db = freshDb();
        db.upsertV3Pool({ address: OTHER_POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 20 });
        db.upsertV3Pool({ address: POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 10 });
        expect(db.listV3Pools().map((pool) => pool.createdBlock)).toEqual([10, 20]);
    });

    it('updates the stored slot0 price in place', () =>
    {
        const db = freshDb();
        db.upsertV3Pool({ address: POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 1 });
        db.updateV3Price(POOL, (1n << 96n));
        expect(db.getV3Pool(POOL)?.sqrtPriceX96).toBe(1n << 96n);
        db.updateV3Price(POOL, (2n << 96n));
        expect(db.getV3Pool(POOL)?.sqrtPriceX96).toBe(2n << 96n);
    });
});
...
describe('events', () =>
{
    ...
    // two different pools can never collide. Same block, different index must
    ...
    {
        const db = freshDb();
        expect(db.insertEvent(event({ logIndex: 0, pair: POOL }))).toBe(true);
        expect(db.insertEvent(event({ logIndex: 1, pair: OTHER_POOL }))).toBe(true);
    ...
    it('normalizes pool and account case on write', () =>
    {
        const db = freshDb();
        db.insertEvent(event({ pair: POOL.toUpperCase() as Address, account: ALICE.toUpperCase() as Address }));
        expect(db.recentEvents(10, { pair: POOL })).toHaveLength(1);
    ...
    it('filters case-insensitively, so a checksummed query still matches', () =>
    {
        ...
        expect(db.recentEvents(10, { pair: POOL.toUpperCase() })).toHaveLength(1);
    });
    ...
    it('combines the pool and account filters with AND', () =>
    {
        const db = freshDb();
        db.insertEvent(event({ logIndex: 1, pair: POOL, account: ALICE }));
        db.insertEvent(event({ logIndex: 2, pair: POOL, account: BOB }));
        db.insertEvent(event({ logIndex: 3, pair: OTHER_POOL, account: ALICE }));
        expect(db.recentEvents(10, { pair: POOL, account: ALICE })).toHaveLength(1);
        expect(db.recentEvents(10, { pair: POOL })).toHaveLength(2);
...
describe('parameter binding is injection-proof', () =>
{
    ...
    it('treats an injection payload in a pool filter as a literal', () =>
    {
        const db = freshDb();
        db.insertEvent(event({ pair: POOL }));
    ...
    it('does not let a lookup drop a table', () =>
    {
        ...
        expect(db.getV3Pool("x'; DROP TABLE v3_pools; --")).toBeNull();
...
describe('candles', () =>
{
    ...
    {
        const db = freshDb();
        db.recordCandlePoint(POOL, HOUR, 100n, 5n, 7n);
    ...
    {
        const db = freshDb();
        db.recordCandlePoint(POOL, HOUR, 100n, 1n, 1n);
        db.recordCandlePoint(POOL, HOUR, 140n, 2n, 2n);
        db.recordCandlePoint(POOL, HOUR, 80n, 3n, 3n);
        db.recordCandlePoint(POOL, HOUR, 120n, 4n, 4n);
        const [candle] = db.candles(POOL, 0);
    ...
    {
        const db = freshDb();
        db.recordCandlePoint(POOL, HOUR, 1n, MAX_UINT256 / 3n, MAX_UINT256 / 3n);
        db.recordCandlePoint(POOL, HOUR, 1n, MAX_UINT256 / 3n, MAX_UINT256 / 3n);
        const [candle] = db.candles(POOL, 0);
        expect(candle.volume0 > (1n << 63n)).toBe(true);
        expect(candle.volume0).toBe((MAX_UINT256 / 3n) * 2n);
    });

    it('keeps hours and pools in separate rows', () =>
    {
        const db = freshDb();
        db.recordCandlePoint(POOL, HOUR, 10n, 1n, 1n);
        db.recordCandlePoint(POOL, 2 * HOUR, 20n, 1n, 1n);
        db.recordCandlePoint(OTHER_POOL, HOUR, 30n, 1n, 1n);
        expect(db.candles(POOL, 0)).toHaveLength(2);
        expect(db.candles(OTHER_POOL, 0)).toHaveLength(1);
        expect(db.candles(OTHER_POOL, 0)[0].open).toBe(30n);
    });

    it('returns candles in chronological order from the requested hour', () =>
    {
        const db = freshDb();
        db.recordCandlePoint(POOL, 3 * HOUR, 30n, 1n, 1n);
        db.recordCandlePoint(POOL, HOUR, 10n, 1n, 1n);
        db.recordCandlePoint(POOL, 2 * HOUR, 20n, 1n, 1n);
        expect(db.candles(POOL, 0).map((candle) => candle.hourStart)).toEqual([HOUR, 2 * HOUR, 3 * HOUR]);
        // The bound is inclusive.
        expect(db.candles(POOL, 2 * HOUR).map((candle) => candle.hourStart)).toEqual([2 * HOUR, 3 * HOUR]);
        expect(db.candles(POOL, 99 * HOUR)).toEqual([]);
    });

    it('is case-insensitive on the pool address', () =>
    {
        const db = freshDb();
        db.recordCandlePoint(POOL.toUpperCase(), HOUR, 10n, 1n, 1n);
        expect(db.candles(POOL, 0)).toHaveLength(1);
    });

    it('reads the last close straight off the primary key', () =>
    {
        const db = freshDb();
        expect(db.latestClose(POOL)).toBeNull();
        db.recordCandlePoint(POOL, HOUR, 10n, 1n, 1n);
        db.recordCandlePoint(POOL, 2 * HOUR, 20n, 1n, 1n);
        expect(db.latestClose(POOL)).toBe(20n);
    });
});
...
describe('volumeSince', () =>
{
    ...
    {
        const db = freshDb();
        db.recordCandlePoint(POOL, HOUR, 1n, 10n, 100n);
        db.recordCandlePoint(POOL, 2 * HOUR, 1n, 20n, 200n);
        db.recordCandlePoint(POOL, 3 * HOUR, 1n, 30n, 300n);
        expect(db.volumeSince(POOL, 0)).toEqual({ volume0: 60n, volume1: 600n });
        // Inclusive lower bound: the 2h candle counts.
        expect(db.volumeSince(POOL, 2 * HOUR)).toEqual({ volume0: 50n, volume1: 500n });
        expect(db.volumeSince(POOL, 4 * HOUR)).toEqual({ volume0: 0n, volume1: 0n });
    });

    it('is zero for a pool that has never traded', () =>
    {
        expect(freshDb().volumeSince(POOL, 0)).toEqual({ volume0: 0n, volume1: 0n });
    });

    it('does not mix one pool volume into another', () =>
    {
        const db = freshDb();
        db.recordCandlePoint(POOL, HOUR, 1n, 10n, 10n);
        db.recordCandlePoint(OTHER_POOL, HOUR, 1n, 99n, 99n);
        expect(db.volumeSince(POOL, 0)).toEqual({ volume0: 10n, volume1: 10n });
    });
});
...
describe('wipe', () =>
{
    it('clears every table, including the identity it was keyed on', () =>
    {
        ...
        db.upsertV3Pool({ address: POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 1 });
        db.insertEvent(event());
        db.recordCandlePoint(POOL, 3600, 1n, 1n, 1n);
    ...
    it('leaves the schema usable, so the re-index can write immediately', () =>
    {
        ...
        db.upsertV3Pool({ address: POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 1 });
        db.wipe();
        db.upsertV3Pool({ address: POOL, token0: NURA, token1: USDT, fee: 500, createdBlock: 2 });
        expect(db.getV3Pool(POOL)?.createdBlock).toBe(2);
    });
});
